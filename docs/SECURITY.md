...
Everything else is a QR code. This page sets out what that means: what counts as
a credential, what the API will and will not serve, and the one place where the
model is knowingly weak.
...
- **No personal data in anonymous responses.** Player names, team QR codes and
  host ids are served only to the game's own host.
...
- **Player names** are generated by the server as an `adjective-animal` handle.
  Players never type one, so no player's real name is in the app.
...
- **Retention.** Ending a game clears the personal data play needed; the whole
  game is deleted after `GAME_RETENTION_DAYS`, sweeper-driven and automatic.
